...
char **parser(char *input) {
  ...
  char **tokens = malloc(buf_size * sizeof(char *)); // allocate memory for tokens
  ...
  while (input[i] != '\0') {
    ...
    for (size_t j = 0; j < token_len; j++) { tokens[position][j] = token[j]; }
...
void free_tokens(char **tokens) {
  if (!tokens) // check if tokens is not null
    return;
  for (size_t i = 0; tokens[i]; i++) free(tokens[i]); // free memory for each token
