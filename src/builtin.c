...
int builtin(char **args, char **env, char *init_dir) {
  for (size_t i = 0; i < BUILTIN_COUNT; i++) {
        ...
        args[0][strlen(builtins[i].name)] == '\0')
    {
      ...
      if (strncmp(args[0], "exit", strlen("exit")) == 0 && args[0][4] == '\0') exit(EXIT_SUCCESS);
...
int echo_cmd(char **args, char **env, char *init_dir) {
  ...
  if (args[1] && !strncmp(args[1], "-n", strlen("-n")) && args[1][2] == '\0') {
    for (size_t i = 2; args[i]; i++) { fprintf(stdout, "%s ", args[i]); }
    return EXIT_SUCCESS;
  } else {
    for (size_t i = 1; args[i]; i++) {
      if (args[i][0] == '$') { // checks env vars
        fprintf(stdout, "%s ", getenv_(args[i] + 1, env));
      } else if (args[i][0] == '"' && args[i][strlen(args[i]) - 1] == '"') { // checks for quotes
        size_t len = strlen(args[i]);
        if (len >= 2) { fprintf(stdout, "%.*s ", (int)(len - 2), args[i] + 1); }
      } else {
...
int which_cmd(char **args, char **env, char *init_dir) {
  (void)init_dir;
  if (!args[1]) { fprintf(stderr, "which: expected arguments\n"); }
  static const char *builtins[] = {"cd", "pwd", "exit", "echo", "alias", "source", "which", "type",
  ...
  char *token = strtok(path_temp, ":"); // strtok: doesnt allocate memory for `token`
